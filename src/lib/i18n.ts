export const LANGUAGES = [
  { code: "en", label: "English", short: "EN", native: "English" },
  { code: "hi", label: "Hindi", short: "हिं", native: "हिन्दी" },
  { code: "te", label: "Telugu", short: "తె", native: "తెలుగు" },
  { code: "ta", label: "Tamil", short: "த", native: "தமிழ்" },
  { code: "kn", label: "Kannada", short: "ಕ", native: "ಕನ್ನಡ" },
  { code: "mr", label: "Marathi", short: "म", native: "मराठी" },
] as const;

export type LanguageCode = (typeof LANGUAGES)[number]["code"];

export const DEFAULT_LANGUAGE: LanguageCode = "en";
export const STORAGE_KEY = "agrilink-language";

type Dict = Record<string, string>;

const hi: Dict = {
  "Buy produce": "उपज खरीदें",
  "Sell produce": "उपज बेचें",
  Learn: "सीखें",
  "Direct produce exchange · India": "प्रत्यक्ष उपज विनिमय · भारत",
  "Better prices start at the farm gate.":
    "बेहतर दाम खेत के द्वार से शुरू होते हैं।",
  "AgriLink gives farmers control of inventory and buyers a clear view of fresh stock, delivery cost, and the people behind every order.":
    "AgriLink किसानों को स्टॉक का नियंत्रण देता है और खरीदारों को ताज़ा स्टॉक, डिलीवरी लागत और हर ऑर्डर के पीछे के लोगों की स्पष्ट जानकारी देता है।",
  "Browse today's harvest": "आज की फसल देखें",
  "List your produce": "अपनी उपज सूचीबद्ध करें",
  "active farm lots": "सक्रिय फार्म लॉट",
  "farmer payouts this week": "इस सप्ताह किसान भुगतान",
  "MARKET UPDATE": "बाज़ार अपडेट",
  "Fresh lots added all day": "दिन भर ताज़ा लॉट जुड़ते हैं",
  "Compare quality, source, and price before buying":
    "खरीदने से पहले गुणवत्ता, स्रोत और दाम की तुलना करें",
  "Fresh off the field": "खेत से ताज़ा",
  "Market board": "बाज़ार बोर्ड",
  "See all listings →": "सभी लिस्टिंग देखें →",
  "Price clarity": "मूल्य स्पष्टता",
  "See regional price baselines and your net payout before you accept.":
    "स्वीकार करने से पहले क्षेत्रीय मूल्य आधार और अपना शुद्ध भुगतान देखें।",
  "Coordinated delivery": "समन्वित डिलीवरी",
  "Transparent distance and multi-stop route estimates for every order.":
    "हर ऑर्डर के लिए पारदर्शी दूरी और मल्टी-स्टॉप रूट अनुमान।",
  "Tracked lifecycle": "ट्रैक किया गया जीवनचक्र",
  "From harvest pack to hand-off to delivery, every update is visible.":
    "कटाई-पैकिंग से हैंडऑफ़ से डिलीवरी तक, हर अपडेट दिखाई देता है।",
  "Live farm inventory · Hyderabad region":
    "लाइव फार्म स्टॉक · हैदराबाद क्षेत्र",
  "Buy closer to the source.": "स्रोत के करीब से खरीदें।",
  "Try: fresh tomatoes under ₹30/kg near Hyderabad":
    "आज़माएँ: हैदराबाद के पास ₹30/किलो से कम ताज़ा टमाटर",
  "Best match": "सर्वोत्तम मिलान",
  "Price: low to high": "मूल्य: कम से ज़्यादा",
  Distance: "दूरी",
  "More filters": "और फ़िल्टर",
  "All produce": "सभी उपज",
  Vegetables: "सब्ज़ियाँ",
  Fruits: "फल",
  Grains: "अनाज",
  Pulses: "दालें",
  Spices: "मसाले",
  Rice: "चावल",
  "Contact number": "संपर्क नंबर",
  "Seller phone · bulk orders": "विक्रेता फ़ोन · थोक ऑर्डर",
  "Call seller for bulk orders": "थोक ऑर्डर के लिए विक्रेता को कॉल करें",
  Harvested: "कटाई",
  "km away": "किमी दूर",
  "Your collection cart": "आपकी संग्रह टोकरी",
  "Adjust quantities, remove items, or continue to a final server-side stock check.":
    "मात्रा बदलें, आइटम हटाएँ, या अंतिम स्टॉक जाँच के लिए आगे बढ़ें।",
  "Your cart is empty.": "आपकी टोकरी खाली है।",
  "Pick freshly listed produce from the market board.":
    "बाज़ार बोर्ड से ताज़ा सूचीबद्ध उपज चुनें।",
  "Browse marketplace": "बाज़ार देखें",
  "Order summary": "ऑर्डर सारांश",
  Produce: "उपज",
  "Delivery estimate": "डिलीवरी अनुमान",
  "Platform fee": "प्लेटफ़ॉर्म शुल्क",
  Total: "कुल",
  "Continue to checkout": "चेकआउट पर जाएँ",
  "Checkout · secure demo flow": "चेकआउट · सुरक्षित डेमो प्रक्रिया",
  "Confirm collection & delivery": "संग्रह और डिलीवरी की पुष्टि करें",
  "Delivery details": "डिलीवरी विवरण",
  "Tomorrow · 9–12": "कल · 9–12",
  "Tomorrow · 2–5": "कल · 2–5",
  Payment: "भुगतान",
  "Demo checkout — no payment details are stored.":
    "डेमो चेकआउट — कोई भुगतान विवरण संग्रहीत नहीं होता।",
  "Order placed successfully.": "ऑर्डर सफलतापूर्वक दिया गया।",
  "Payment record AGRPAY-90217 saved. Farmer confirmation is next.":
    "भुगतान रिकॉर्ड AGRPAY-90217 सहेजा गया। आगे किसान की पुष्टि होगी।",
  "Track order": "ऑर्डर ट्रैक करें",
  "Cash collection is pending on delivery. Your order can proceed.":
    "डिलीवरी पर नकद संग्रह लंबित है। आपका ऑर्डर आगे बढ़ सकता है।",
  "Simulate payment": "भुगतान का अनुकरण करें",
  "Place order": "ऑर्डर दें",
  "SELL PRODUCE": "उपज बेचें",
  "Your selling board": "आपका विक्रय बोर्ड",
  "Any AgriLink member can list fresh produce and manage their own sales.":
    "कोई भी AgriLink सदस्य ताज़ा उपज सूचीबद्ध कर सकता है और अपनी बिक्री संभाल सकता है।",
  "New listing": "नई लिस्टिंग",
  "Active listings": "सक्रिय लिस्टिंग",
  "Est. earnings": "अनुमानित कमाई",
  "Orders in progress": "प्रगति में ऑर्डर",
  "Stock running low": "स्टॉक कम हो रहा है",
  "Your active stock": "आपका सक्रिय स्टॉक",
  "Manage stock": "स्टॉक प्रबंधित करें",
  "Price advisory": "मूल्य सलाह",
  "Tomato market position": "टमाटर बाज़ार स्थिति",
  "your listing / kg": "आपकी लिस्टिंग / किलो",
  "Set a location for nearby prices and delivery estimates":
    "आस-पास के दाम और डिलीवरी अनुमान के लिए स्थान निर्धारित करें",
  "You can skip this and explore every market price first.":
    "आप इसे छोड़कर पहले सभी बाज़ार भाव देख सकते हैं।",
  "Set location": "स्थान निर्धारित करें",
  "Not now": "अभी नहीं",
  "Save location": "स्थान सहेजें",
  "Village, district, state, or pincode": "गाँव, ज़िला, राज्य या पिनकोड",
  "Sign in": "साइन इन",
  "Log out": "लॉग आउट",
  "AGRLINK ACCESS": "एग्रीलिंक एक्सेस",
  "Sign in to your desk": "अपने डेस्क पर साइन इन करें",
  "New to AgriLink?": "AgriLink पर नए हैं?",
  "Create an account": "खाता बनाएँ",
  "CREATE ACCOUNT": "खाता बनाएँ",
  "Join the open market": "खुले बाज़ार से जुड़ें",
  "Full name": "पूरा नाम",
  "Email address": "ईमेल पता",
  "Mobile number": "मोबाइल नंबर",
  Password: "पासवर्ड",
  "Every account can buy and sell. Add a location when you are ready to publish produce or set delivery preferences.":
    "हर खाता खरीद और बेच सकता है। उपज प्रकाशित करने या डिलीवरी वरीयता तय करने के लिए स्थान जोड़ें।",
  "Create account": "खाता बनाएँ",
  "Knowledge hub": "ज्ञान केंद्र",
  QUALITY: "गुणवत्ता",
  HARVEST: "कटाई",
  STOCK: "स्टॉक",
  per: "प्रति",
  Quantity: "मात्रा",
  "Add to cart": "टोकरी में जोड़ें",
  "Added to cart": "टोकरी में जोड़ा गया",
  Inquiry: "पूछताछ",
  "Minimum bulk order: 20 kg. Stock is verified again at checkout to protect against orders placed by other buyers.":
    "न्यूनतम थोक ऑर्डर: 20 किलो। अन्य खरीदारों के ऑर्डर से बचाव के लिए चेकआउट पर स्टॉक फिर से सत्यापित होता है।",
  "Platform-coordinated delivery": "प्लेटफ़ॉर्म-समन्वित डिलीवरी",
  "route estimate only": "केवल रूट अनुमान",
  "More from the market board": "बाज़ार बोर्ड से और",
  "You may also need": "आपको यह भी चाहिए हो सकता है",
  "Similar quality, nearby harvests, and complementary produce.":
    "समान गुणवत्ता, आस-पास की कटाई और पूरक उपज।",
  Placed: "दर्ज",
  "Confirmed by farmer": "किसान द्वारा पुष्ट",
  "Harvest & pack": "कटाई और पैकिंग",
  "Handed to transporter": "ट्रांसपोर्टर को सौंपा",
  "In transit": "रास्ते में",
  Delivered: "वितरित",
  Completed: "पूर्ण",
  "Awaiting update": "अपडेट प्रतीक्षित",
  "Produce variety": "उपज की किस्म",
  Unit: "इकाई",
  Grade: "ग्रेड",
  "Available from": "उपलब्ध (से)",
  "Available until": "उपलब्ध (तक)",
  "Expected price / kg": "अपेक्षित मूल्य / किलो",
  "Publish listing": "लिस्टिंग प्रकाशित करें",
  "Quantity must be greater than zero.": "मात्रा शून्य से अधिक होनी चाहिए।",
  "Available-until must be after available-from and cannot be in the past.":
    "उपलब्ध-तक तिथि उपलब्ध-से के बाद होनी चाहिए और भूतकाल में नहीं हो सकती।",
  "Regional suggested price: ₹30/kg": "क्षेत्रीय सुझाया मूल्य: ₹30/किलो",
  "Choose language": "भाषा चुनें",
  Cart: "टोकरी",
  "Your produce is being packed": "आपकी उपज पैक की जा रही है",
  "Collection expected 14:30–15:15": "संग्रह अपेक्षित 14:30–15:15",
  "A transporter will be assigned after the farmer marks your order ready.":
    "किसान द्वारा ऑर्डर तैयार चिह्नित करने के बाद ट्रांसपोर्टर नियुक्त होगा।",
  "DELIVERY ADDRESS": "डिलीवरी पता",
  "Route shown is an estimate, not live GPS.":
    "दिखाया गया रूट अनुमान है, लाइव GPS नहीं।",
  "Publish harvest availability": "फसल उपलब्धता प्रकाशित करें",
  "FARMER DESK / NEW LISTING": "किसान डेस्क / नई लिस्टिंग",
  "Farm location: Chevella, Telangana. Complete your profile before publishing if this is not correct.":
    "फार्म स्थान: चेवेल्ला, तेलंगाना। यदि यह सही नहीं है तो प्रकाशन से पहले प्रोफ़ाइल पूरी करें।",
  "Agriculture field notes": "कृषि जानकारी",
  "Practical market, crop, and weather guidance for producers and buyers.":
    "उत्पादकों और खरीदारों के लिए बाज़ार, फसल और मौसम की व्यावहारिक सलाह।",
  "{count} live listings · data updated 8 min ago":
    "{count} लाइव लिस्टिंग · 8 मिनट पहले अपडेट",
  "No produce matches those filters.": "इन फ़िल्टरों से कोई उपज मेल नहीं खाती।",
  "Broaden your location, price, or freshness filter to see live farm stock.":
    "लाइव फार्म स्टॉक देखने के लिए स्थान, मूल्य या ताज़गी फ़िल्टर बढ़ाएँ।",
  Delete: "हटाएँ",
  "Delete this listing permanently? This cannot be undone.":
    "यह लिस्टिंग स्थायी रूप से हटाएँ? यह क्रिया वापस नहीं होगी।",
  "No listings yet. Publish your first harvest to appear here.":
    "अभी कोई लिस्टिंग नहीं है। यहाँ दिखने के लिए अपनी पहली फसल प्रकाशित करें।",
  "Price must be greater than zero.": "मूल्य शून्य से अधिक होना चाहिए।",
  Yours: "आपका",
  Category: "श्रेणी",
  "Clear cart": "टोकरी साफ़ करें",
  "Continue shopping": "खरीदारी जारी रखें",
  "{count} items": "{count} वस्तुएँ",
  "Delivery slot": "डिलीवरी स्लॉट",
  "Decrease quantity": "मात्रा घटाएँ",
  "Increase quantity": "मात्रा बढ़ाएँ",
  "Remove from cart": "टोकरी से हटाएँ",
  "Payment did not complete. Your order is still placed; retry safely.":
    "भुगतान पूरा नहीं हुआ। आपका ऑर्डर बना हुआ है; सुरक्षित रूप से पुनः प्रयास करें।",
  Analytics: "एनालिटिक्स",
  "View analytics": "एनालिटिक्स देखें",
  "Stock value overview": "स्टॉक मूल्य अवलोकन",
  "Live market data across every visible listing.":
    "प्रत्येक दृश्य लिस्टिंग का लाइव बाज़ार डेटा।",
  "Total stock value": "कुल स्टॉक मूल्य",
  "Paddy stock value": "धान स्टॉक मूल्य",
  "Vegetables stock value": "सब्ज़ी स्टॉक मूल्य",
  "Share of total stock": "कुल स्टॉक का हिस्सा",
  "Paddy vs vegetables": "धान बनाम सब्ज़ियाँ",
  "Other produce": "अन्य उपज",
  "Stock value by category": "श्रेणी अनुसार स्टॉक मूल्य",
  "Top listings by stock value": "स्टॉक मूल्य के शीर्ष लिस्टिंग",
  Value: "मूल्य",
  "AI assistant": "एआई सहायक",
  "Ask about prices, delivery, orders…":
    "दाम, डिलीवरी, ऑर्डर के बारे में पूछें…",
  "Online · replies instantly": "ऑनलाइन · तुरंत उत्तर",
  "Paddy prices": "धान के भाव",
  "How to sell": "कैसे बेचें",
  "Report this listing": "इस लिस्टिंग की रिपोर्ट करें",
  Reported: "रिपोर्ट किया गया",
  "Why are you reporting this listing?":
    "आप इस लिस्टिंग की रिपोर्ट क्यों कर रहे हैं?",
  "Wrong price": "गलत मूल्य",
  "Poor quality": "खराब गुणवत्ता",
  "Sold out": "स्टॉक समाप्त",
  "Spam or fraud": "स्पैम या धोखाधड़ी",
  "Tell us more (optional)": "हमें और बताएँ (वैकल्पिक)",
  Cancel: "रद्द करें",
  "Submit report": "रिपोर्ट जमा करें",
  "Thanks — our team will review this listing.":
    "धन्यवाद — हमारी टीम इस लिस्टिंग की समीक्षा करेगी।",
};

const te: Dict = {
  "Buy produce": "పంట కొనండి",
  "Sell produce": "పంట అమ్మండి",
  Learn: "నేర్చుకోండి",
  "Direct produce exchange · India": "ప్రత్యక్ష పంట మార్పిడి · భారతదేశం",
  "Better prices start at the farm gate.":
    "మెరుగైన ధరలు పొలం గేటు వద్దే మొదలవుతాయి.",
  "AgriLink gives farmers control of inventory and buyers a clear view of fresh stock, delivery cost, and the people behind every order.":
    "AgriLink రైతులకు నిల్వపై నియంత్రణ ఇస్తుంది, కొనుగోలుదారులకు తాజా స్టాక్, డెలివరీ ఖర్చు, ప్రతి ఆర్డర్ వెనుక ఉన్న వ్యక్తులపై స్పష్టత ఇస్తుంది.",
  "Browse today's harvest": "నేటి పంటను చూడండి",
  "List your produce": "మీ పంటను జాబితా చేయండి",
  "active farm lots": "క్రియాశీల పొలం లాట్లు",
  "farmer payouts this week": "ఈ వారం రైతు చెల్లింపులు",
  "MARKET UPDATE": "మార్కెట్ అప్‌డేట్",
  "Fresh lots added all day": "రోజంతా తాజా లాట్లు జోడించబడతాయి",
  "Compare quality, source, and price before buying":
    "కొనే ముందు నాణ్యత, మూలం, ధరను పోల్చండి",
  "Fresh off the field": "పొలం నుండి తాజాగా",
  "Market board": "మార్కెట్ బోర్డు",
  "See all listings →": "అన్ని జాబితాలను చూడండి →",
  "Price clarity": "ధర స్పష్టత",
  "See regional price baselines and your net payout before you accept.":
    "అంగీకరించే ముందు ప్రాంతీయ ధర ప్రమాణాలు, మీ నికర చెల్లింపు చూడండి.",
  "Coordinated delivery": "సమన్వయ డెలివరీ",
  "Transparent distance and multi-stop route estimates for every order.":
    "ప్రతి ఆర్డర్‌కు పారదర్శక దూరం, మల్టీ-స్టాప్ రూట్ అంచనాలు.",
  "Tracked lifecycle": "ట్రాక్ చేయబడిన జీవనచక్రం",
  "From harvest pack to hand-off to delivery, every update is visible.":
    "కోత-ప్యాకింగ్ నుండి అప్పగింత నుండి డెలివరీ వరకు ప్రతి అప్‌డేట్ కనిపిస్తుంది.",
  "Live farm inventory · Hyderabad region":
    "ప్రత్యక్ష పొలం నిల్వ · హైదరాబాద్ ప్రాంతం",
  "Buy closer to the source.": "మూలానికి దగ్గరగా కొనండి.",
  "Try: fresh tomatoes under ₹30/kg near Hyderabad":
    "ప్రయత్నించండి: హైదరాబాద్ సమీపంలో ₹30/కిలో లోపు తాజా టమాటాలు",
  "Best match": "ఉత్తమ సరిపోలిక",
  "Price: low to high": "ధర: తక్కువ నుండి ఎక్కువ",
  Distance: "దూరం",
  "More filters": "మరిన్ని ఫిల్టర్లు",
  "All produce": "అన్ని పంటలు",
  Vegetables: "కూరగాయలు",
  Fruits: "పండ్లు",
  Grains: "ధాన్యాలు",
  Pulses: "పప్పులు",
  Spices: "సుగంధ ద్రవ్యాలు",
  Rice: "బియ్యం",
  "Contact number": "సంప్రదింపు నంబర్",
  "Seller phone · bulk orders": "విక్రేత ఫోన్ · టోకు ఆర్డర్లు",
  "Call seller for bulk orders": "టోకు ఆర్డర్ల కోసం విక్రేతకు కాల్ చేయండి",
  Harvested: "కోత",
  "km away": "కిమీ దూరంలో",
  "Your collection cart": "మీ సేకరణ బండి",
  "Adjust quantities, remove items, or continue to a final server-side stock check.":
    "పరిమాణాలు మార్చండి, అంశాలు తొలగించండి లేదా తుది స్టాక్ తనిఖీకి కొనసాగండి.",
  "Your cart is empty.": "మీ బండి ఖాళీగా ఉంది.",
  "Pick freshly listed produce from the market board.":
    "మార్కెట్ బోర్డు నుండి తాజా పంటను ఎంచుకోండి.",
  "Browse marketplace": "మార్కెట్‌ను చూడండి",
  "Order summary": "ఆర్డర్ సారాంశం",
  Produce: "పంట",
  "Delivery estimate": "డెలివరీ అంచనా",
  "Platform fee": "ప్లాట్‌ఫారమ్ రుసుము",
  Total: "మొత్తం",
  "Continue to checkout": "చెక్అవుట్‌కు వెళ్లండి",
  "Checkout · secure demo flow": "చెక్అవుట్ · సురక్షిత డెమో ప్రక్రియ",
  "Confirm collection & delivery": "సేకరణ & డెలివరీని నిర్ధారించండి",
  "Delivery details": "డెలివరీ వివరాలు",
  "Tomorrow · 9–12": "రేపు · 9–12",
  "Tomorrow · 2–5": "రేపు · 2–5",
  Payment: "చెల్లింపు",
  "Demo checkout — no payment details are stored.":
    "డెమో చెక్అవుట్ — చెల్లింపు వివరాలు నిల్వ చేయబడవు.",
  "Order placed successfully.": "ఆర్డర్ విజయవంతంగా పెట్టబడింది.",
  "Payment record AGRPAY-90217 saved. Farmer confirmation is next.":
    "చెల్లింపు రికార్డు AGRPAY-90217 భద్రపరచబడింది. తర్వాత రైతు నిర్ధారణ.",
  "Track order": "ఆర్డర్ ట్రాక్ చేయండి",
  "Cash collection is pending on delivery. Your order can proceed.":
    "డెలివరీపై నగదు వసూలు పెండింగ్‌లో ఉంది. మీ ఆర్డర్ కొనసాగవచ్చు.",
  "Simulate payment": "చెల్లింపును అనుకరించండి",
  "Place order": "ఆర్డర్ పెట్టండి",
  "SELL PRODUCE": "పంట అమ్మండి",
  "Your selling board": "మీ విక్రయ బోర్డు",
  "Any AgriLink member can list fresh produce and manage their own sales.":
    "ఏ AgriLink సభ్యుడైనా తాజా పంటను జాబితా చేసి సొంత అమ్మకాలను నిర్వహించవచ్చు.",
  "New listing": "కొత్త జాబితా",
  "Active listings": "క్రియాశీల జాబితాలు",
  "Est. earnings": "అంచనా ఆదాయం",
  "Orders in progress": "పురోగతిలో ఆర్డర్లు",
  "Stock running low": "స్టాక్ తగ్గుతోంది",
  "Your active stock": "మీ క్రియాశీల స్టాక్",
  "Manage stock": "స్టాక్ నిర్వహించండి",
  "Price advisory": "ధర సలహా",
  "Tomato market position": "టమాటా మార్కెట్ స్థితి",
  "your listing / kg": "మీ జాబితా / కిలో",
  "Set a location for nearby prices and delivery estimates":
    "సమీప ధరలు, డెలివరీ అంచనాల కోసం స్థానాన్ని సెట్ చేయండి",
  "You can skip this and explore every market price first.":
    "దీన్ని దాటవేసి ముందు అన్ని మార్కెట్ ధరలను చూడవచ్చు.",
  "Set location": "స్థానాన్ని సెట్ చేయండి",
  "Not now": "ఇప్పుడు వద్దు",
  "Save location": "స్థానాన్ని భద్రపరచండి",
  "Village, district, state, or pincode":
    "గ్రామం, జిల్లా, రాష్ట్రం లేదా పిన్‌కోడ్",
  "Sign in": "సైన్ ఇన్",
  "Log out": "లాగ్ అవుట్",
  "AGRLINK ACCESS": "ఎగ్రిలింక్ యాక్సెస్",
  "Sign in to your desk": "మీ డెస్క్‌కు సైన్ ఇన్ చేయండి",
  "New to AgriLink?": "AgriLinkకు కొత్తవారా?",
  "Create an account": "ఖాతా సృష్టించండి",
  "CREATE ACCOUNT": "ఖాతా సృష్టించండి",
  "Join the open market": "బహిరంగ మార్కెట్‌లో చేరండి",
  "Full name": "పూర్తి పేరు",
  "Email address": "ఈమెయిల్ చిరునామా",
  "Mobile number": "మొబైల్ నంబర్",
  Password: "పాస్‌వర్డ్",
  "Every account can buy and sell. Add a location when you are ready to publish produce or set delivery preferences.":
    "ప్రతి ఖాతా కొనవచ్చు, అమ్మవచ్చు. పంటను ప్రచురించడానికి లేదా డెలివరీ ప్రాధాన్యతలు సెట్ చేయడానికి స్థానాన్ని జోడించండి.",
  "Create account": "ఖాతా సృష్టించండి",
  "Knowledge hub": "జ్ఞాన కేంద్రం",
  QUALITY: "నాణ్యత",
  HARVEST: "కోత",
  STOCK: "స్టాక్",
  per: "కు",
  Quantity: "పరిమాణం",
  "Add to cart": "బండిలో చేర్చండి",
  "Added to cart": "బండిలో చేర్చబడింది",
  Inquiry: "విచారణ",
  "Minimum bulk order: 20 kg. Stock is verified again at checkout to protect against orders placed by other buyers.":
    "కనీస టోకు ఆర్డర్: 20 కిలోలు. ఇతర కొనుగోలుదారుల ఆర్డర్ల నుండి రక్షణకు చెక్అవుట్ వద్ద స్టాక్ మళ్లీ ధృవీకరించబడుతుంది.",
  "Platform-coordinated delivery": "ప్లాట్‌ఫారమ్-సమన్వయ డెలివరీ",
  "route estimate only": "రూట్ అంచనా మాత్రమే",
  "More from the market board": "మార్కెట్ బోర్డు నుండి మరిన్ని",
  "You may also need": "మీకు ఇవి కూడా అవసరం కావచ్చు",
  "Similar quality, nearby harvests, and complementary produce.":
    "సమాన నాణ్యత, సమీప కోతలు, పૂరక పంటలు.",
  Placed: "నమోదైంది",
  "Confirmed by farmer": "రైతు ధృవీకరించారు",
  "Harvest & pack": "కోత & ప్యాకింగ్",
  "Handed to transporter": "రవాణాదారుకు అప్పగించారు",
  "In transit": "రవాణాలో",
  Delivered: "డెలివరీ అయింది",
  Completed: "పూర్తయింది",
  "Awaiting update": "అప్‌డేట్ కోసం వేచి ఉంది",
  "Produce variety": "పంట రకం",
  Unit: "యూనిట్",
  Grade: "గ్రేడ్",
  "Available from": "అందుబాటు (నుండి)",
  "Available until": "అందుబాటు (వరకు)",
  "Expected price / kg": "ఆశించిన ధర / కిలో",
  "Publish listing": "జాబితాను ప్రచురించండి",
  "Quantity must be greater than zero.": "పరిమాణం సున్నా కంటే ఎక్కువగా ఉండాలి.",
  "Available-until must be after available-from and cannot be in the past.":
    "అందుబాటు-వరకు తేదీ అందుబాటు-నుండి తర్వాత ఉండాలి, గతంలో ఉండకూడదు.",
  "Regional suggested price: ₹30/kg": "ప్రాంతీయ సూచించిన ధర: ₹30/కిలో",
  "Choose language": "భాషను ఎంచుకోండి",
  Cart: "బండి",
  "Your produce is being packed": "మీ పంట ప్యాక్ చేయబడుతోంది",
  "Collection expected 14:30–15:15": "సేకరణ అంచనా 14:30–15:15",
  "A transporter will be assigned after the farmer marks your order ready.":
    "రైతు మీ ఆర్డర్ సిద్ధంగా గుర్తించిన తర్వాత రవాణాదారు కేటాయించబడతారు.",
  "DELIVERY ADDRESS": "డెలివరీ చిరునామా",
  "Route shown is an estimate, not live GPS.":
    "చూపిన రూట్ అంచనా మాత్రమే, ప్రత్యక్ష GPS కాదు.",
  "Publish harvest availability": "పంట లభ్యతను ప్రచురించండి",
  "FARMER DESK / NEW LISTING": "రైతు డెస్క్ / కొత్త జాబితా",
  "Farm location: Chevella, Telangana. Complete your profile before publishing if this is not correct.":
    "పొలం స్థానం: చేవెళ్ల, తెలంగాణ. ఇది సరికాకపోతే ప్రచురించే ముందు ప్రొఫైల్ పూర్తి చేయండి.",
  "Agriculture field notes": "వ్యవసాయ సమాచారం",
  "Practical market, crop, and weather guidance for producers and buyers.":
    "ఉత్పత్తిదారులు, కొనుగోలుదారుల కోసం మార్కెట్, పంట, వాతావరణ ఆచరణ సలహాలు.",
  "{count} live listings · data updated 8 min ago":
    "{count} ప్రత్యక్ష జాబితాలు · 8 నిమిషాల క్రితం నవీకరించబడింది",
  "No produce matches those filters.": "ఈ ఫిల్టర్లకు ఏ పంట సరిపోలలేదు.",
  "Broaden your location, price, or freshness filter to see live farm stock.":
    "ప్రత్యక్ష పొలం స్టాక్ చూడటానికి స్థానం, ధర లేదా తాజాదనం ఫిల్టర్ విస్తరించండి.",
  Delete: "తొలగించు",
  "Delete this listing permanently? This cannot be undone.":
    "ఈ జాబితాను శాశ్వతంగా తొలగించాలా? దీన్ని రద్దు చేయలేరు.",
  "No listings yet. Publish your first harvest to appear here.":
    "ఇంకా జాబితాలు లేవు. ఇక్కడ కనిపించడానికి మీ మొదటి పంటను ప్రచురించండి.",
  "Price must be greater than zero.": "ధర సున్నా కంటే ఎక్కువగా ఉండాలి.",
  Yours: "మీది",
  Category: "వర్గం",
  "Clear cart": "బండిని ఖాళీ చేయండి",
  "Continue shopping": "షాపింగ్ కొనసాగించండి",
  "{count} items": "{count} వస్తువులు",
  "Delivery slot": "డెలివరీ స్లాట్",
  "Decrease quantity": "పరిమాణం తగ్గించండి",
  "Increase quantity": "పరిమాణం పెంచండి",
  "Remove from cart": "బండి నుండి తొలగించండి",
  "Payment did not complete. Your order is still placed; retry safely.":
    "చెల్లింపు పూర్తి కాలేదు. మీ ఆర్డర్ అలాగే ఉంది; సురక్షితంగా మళ్లీ ప్రయత్నించండి.",
  Analytics: "విశ్లేషణ",
  "View analytics": "విశ్లేషణ చూడండి",
  "Stock value overview": "స్టాక్ విలువ సారాంశం",
  "Live market data across every visible listing.":
    "ప్రతి కనిపించే జాబితా యొక్క ప్రత్యక్ష మార్కెట్ డేటా.",
  "Total stock value": "మొత్తం స్టాక్ విలువ",
  "Paddy stock value": "వరి స్టాక్ విలువ",
  "Vegetables stock value": "కూరగాయల స్టాక్ విలువ",
  "Share of total stock": "మొత్తం స్టాక్‌లో వాటా",
  "Paddy vs vegetables": "వరి vs కూరగాయలు",
  "Other produce": "ఇతర పంటలు",
  "Stock value by category": "వర్గాల వారీగా స్టాక్ విలువ",
  "Top listings by stock value": "స్టాక్ విలువ ప్రకారం అగ్ర జాబితాలు",
  Value: "విలువ",
  "AI assistant": "AI సహాయకుడు",
  "Ask about prices, delivery, orders…":
    "ధరలు, డెలివరీ, ఆర్డర్ల గురించి అడగండి…",
  "Online · replies instantly": "ఆన్‌లైన్ · తక్షణ సమాధానం",
  "Paddy prices": "వరి ధరలు",
  "How to sell": "ఎలా అమ్మాలి",
  "Report this listing": "ఈ జాబితాను నివేదించండి",
  Reported: "నివేదించబడింది",
  "Why are you reporting this listing?":
    "మీరు ఈ జాబితాను ఎందుకు నివేదిస్తున్నారు?",
  "Wrong price": "తప్పు ధర",
  "Poor quality": "తక్కువ నాణ్యత",
  "Sold out": "స్టాక్ అయిపోయింది",
  "Spam or fraud": "స్పామ్ లేదా మోసం",
  "Tell us more (optional)": "మరింత చెప్పండి (ఐచ్ఛికం)",
  Cancel: "రద్దు చేయండి",
  "Submit report": "నివేదికను సమర్పించండి",
  "Thanks — our team will review this listing.":
    "ధన్యవాదాలు — మా బృందం ఈ జాబితాను సమీక్షిస్తుంది.",
};

const ta: Dict = {
  "Buy produce": "விளைபொருள் வாங்க",
  "Sell produce": "விளைபொருள் விற்க",
  Learn: "கற்க",
  "Direct produce exchange · India": "நேரடி விளைபொருள் பரிமாற்றம் · இந்தியா",
  "Better prices start at the farm gate.":
    "சிறந்த விலைகள் பண்ணை வாயிலில் தொடங்குகின்றன.",
  "AgriLink gives farmers control of inventory and buyers a clear view of fresh stock, delivery cost, and the people behind every order.":
    "AgriLink விவசாயிகளுக்கு இருப்பு கட்டுப்பாட்டையும், வாங்குபவர்களுக்கு புதிய இருப்பு, விநியோக செலவு, ஒவ்வொரு ஆர்டருக்கும் பின்னால் உள்ளவர்கள் பற்றிய தெளிவையும் தருகிறது.",
  "Browse today's harvest": "இன்றைய அறுவடையைப் பார்க்க",
  "List your produce": "உங்கள் விளைபொருளைப் பட்டியலிடுக",
  "active farm lots": "செயலில் உள்ள பண்ணை தொகுதிகள்",
  "farmer payouts this week": "இந்த வார விவசாயி கொடுப்பனவுகள்",
  "MARKET UPDATE": "சந்தை நிலவரம்",
  "Fresh lots added all day": "நாள் முழுவதும் புதிய தொகுதிகள் சேரும்",
  "Compare quality, source, and price before buying":
    "வாங்கும் முன் தரம், மூலம், விலையை ஒப்பிடுங்கள்",
  "Fresh off the field": "வயலில் இருந்து புதிதாக",
  "Market board": "சந்தை பலகை",
  "See all listings →": "அனைத்து பட்டியல்களையும் காண →",
  "Price clarity": "விலை தெளிவு",
  "See regional price baselines and your net payout before you accept.":
    "ஏற்கும் முன் பிராந்திய விலை அடிப்படைகளையும் உங்கள் நிகர தொகையையும் காணுங்கள்.",
  "Coordinated delivery": "ஒருங்கிணைந்த விநியோகம்",
  "Transparent distance and multi-stop route estimates for every order.":
    "ஒவ்வொரு ஆர்டருக்கும் வெளிப்படையான தூரம், பல நிறுத்த வழி மதிப்பீடுகள்.",
  "Tracked lifecycle": "கண்காணிக்கப்பட்ட வாழ்க்கைச் சுழற்சி",
  "From harvest pack to hand-off to delivery, every update is visible.":
    "அறுவடை-பொதியிலிருந்து ஒப்படைப்பு முதல் விநியோகம் வரை ஒவ்வொரு புதுப்பிப்பும் தெரியும்.",
  "Live farm inventory · Hyderabad region":
    "நேரடி பண்ணை இருப்பு · ஹைதராபாத் பகுதி",
  "Buy closer to the source.": "மூலத்திற்கு அருகில் வாங்குங்கள்.",
  "Try: fresh tomatoes under ₹30/kg near Hyderabad":
    "முயலுங்கள்: ஹைதராபாத் அருகில் ₹30/கிலோவுக்குள் புதிய தக்காளி",
  "Best match": "சிறந்த பொருத்தம்",
  "Price: low to high": "விலை: குறைவு முதல் அதிகம்",
  Distance: "தூரம்",
  "More filters": "மேலும் வடிப்பான்கள்",
  "All produce": "அனைத்து விளைபொருட்கள்",
  Vegetables: "காய்கறிகள்",
  Fruits: "பழங்கள்",
  Grains: "தானியங்கள்",
  Pulses: "பருப்புகள்",
  Spices: "மசாலாக்கள்",
  Rice: "அரிசி",
  "Contact number": "தொடர்பு எண்",
  "Seller phone · bulk orders": "விற்பனையாளர் தொலைபேசி · மொத்த ஆர்டர்கள்",
  "Call seller for bulk orders": "மொத்த ஆர்டர்களுக்கு விற்பனையாளரை அழைக்கவும்",
  Harvested: "அறுவடை",
  "km away": "கிமீ தொலைவில்",
  "Your collection cart": "உங்கள் சேகரிப்பு கூடை",
  "Adjust quantities, remove items, or continue to a final server-side stock check.":
    "அளவுகளை மாற்றுங்கள், உருப்படிகளை நீக்குங்கள் அல்லது இறுதி இருப்பு சரிபார்ப்புக்கு தொடருங்கள்.",
  "Your cart is empty.": "உங்கள் கூடை காலியாக உள்ளது.",
  "Pick freshly listed produce from the market board.":
    "சந்தை பலகையில் இருந்து புதிய விளைபொருட்களைத் தேர்ந்தெடுங்கள்.",
  "Browse marketplace": "சந்தையைப் பார்க்க",
  "Order summary": "ஆர்டர் சுருக்கம்",
  Produce: "விளைபொருள்",
  "Delivery estimate": "விநியோக மதிப்பீடு",
  "Platform fee": "தள கட்டணம்",
  Total: "மொத்தம்",
  "Continue to checkout": "பணம் செலுத்தச் செல்க",
  "Checkout · secure demo flow": "செக்அவுட் · பாதுகாப்பான டெமோ முறை",
  "Confirm collection & delivery": "சேகரிப்பு & விநியோகத்தை உறுதி செய்க",
  "Delivery details": "விநியோக விவரங்கள்",
  "Tomorrow · 9–12": "நாளை · 9–12",
  "Tomorrow · 2–5": "நாளை · 2–5",
  Payment: "கட்டணம்",
  "Demo checkout — no payment details are stored.":
    "டெமோ செக்அவுட் — கட்டண விவரங்கள் சேமிக்கப்படாது.",
  "Order placed successfully.": "ஆர்டர் வெற்றிகரமாக இடப்பட்டது.",
  "Payment record AGRPAY-90217 saved. Farmer confirmation is next.":
    "கட்டண பதிவு AGRPAY-90217 சேமிக்கப்பட்டது. அடுத்து விவசாயி உறுதிப்படுத்துவார்.",
  "Track order": "ஆர்டரைக் கண்காணி",
  "Cash collection is pending on delivery. Your order can proceed.":
    "விநியோகத்தின் போது ரொக்க வசூல் நிலுவையில் உள்ளது. உங்கள் ஆர்டர் தொடரலாம்.",
  "Simulate payment": "கட்டணத்தை உருவகப்படுத்து",
  "Place order": "ஆர்டர் இடு",
  "SELL PRODUCE": "விளைபொருள் விற்க",
  "Your selling board": "உங்கள் விற்பனை பலகை",
  "Any AgriLink member can list fresh produce and manage their own sales.":
    "எந்த AgriLink உறுப்பினரும் புதிய விளைபொருட்களைப் பட்டியலிட்டு சொந்த விற்பனையை நிர்வகிக்கலாம்.",
  "New listing": "புதிய பட்டியல்",
  "Active listings": "செயலில் உள்ள பட்டியல்கள்",
  "Est. earnings": "மதிப்பிடப்பட்ட வருமானம்",
  "Orders in progress": "நடைபெறும் ஆர்டர்கள்",
  "Stock running low": "இருப்பு குறைந்து வருகிறது",
  "Your active stock": "உங்கள் செயலில் உள்ள இருப்பு",
  "Manage stock": "இருப்பை நிர்வகி",
  "Price advisory": "விலை ஆலோசனை",
  "Tomato market position": "தக்காளி சந்தை நிலை",
  "your listing / kg": "உங்கள் பட்டியல் / கிலோ",
  "Set a location for nearby prices and delivery estimates":
    "அருகிலுள்ள விலைகள், விநியோக மதிப்பீடுகளுக்கு இடத்தை அமைக்கவும்",
  "You can skip this and explore every market price first.":
    "இதைத் தவிர்த்து முதலில் அனைத்து சந்தை விலைகளையும் பார்க்கலாம்.",
  "Set location": "இடத்தை அமை",
  "Not now": "இப்போது வேண்டாம்",
  "Save location": "இடத்தைச் சேமி",
  "Village, district, state, or pincode":
    "கிராமம், மாவட்டம், மாநிலம் அல்லது பின்கோடு",
  "Sign in": "உள்நுழை",
  "Log out": "வெளியேறு",
  "AGRLINK ACCESS": "அக்ரிலிங்க் அணுகல்",
  "Sign in to your desk": "உங்கள் மேசையில் உள்நுழையுங்கள்",
  "New to AgriLink?": "AgriLink-க்கு புதியவரா?",
  "Create an account": "கணக்கை உருவாக்குங்கள்",
  "CREATE ACCOUNT": "கணக்கை உருவாக்குங்கள்",
  "Join the open market": "திறந்த சந்தையில் சேருங்கள்",
  "Full name": "முழு பெயர்",
  "Email address": "மின்னஞ்சல் முகவரி",
  "Mobile number": "கைபேசி எண்",
  Password: "கடவுச்சொல்",
  "Every account can buy and sell. Add a location when you are ready to publish produce or set delivery preferences.":
    "ஒவ்வொரு கணக்கும் வாங்கலாம், விற்கலாம். விளைபொருளை வெளியிட அல்லது விநியோக விருப்பங்களை அமைக்க தயாராகும்போது இடத்தைச் சேர்க்கவும்.",
  "Create account": "கணக்கை உருவாக்கு",
  "Knowledge hub": "அறிவு மையம்",
  QUALITY: "தரம்",
  HARVEST: "அறுவடை",
  STOCK: "இருப்பு",
  per: "க்கு",
  Quantity: "அளவு",
  "Add to cart": "கூடையில் சேர்",
  "Added to cart": "கூடையில் சேர்க்கப்பட்டது",
  Inquiry: "விசாரணை",
  "Minimum bulk order: 20 kg. Stock is verified again at checkout to protect against orders placed by other buyers.":
    "குறைந்தபட்ச மொத்த ஆர்டர்: 20 கிலோ. மற்ற வாங்குபவர்களின் ஆர்டர்களிலிருந்து பாதுகாக்க செக்அவுட்டில் இருப்பு மீண்டும் சரிபார்க்கப்படும்.",
  "Platform-coordinated delivery": "தள-ஒருங்கிணைந்த விநியோகம்",
  "route estimate only": "வழி மதிப்பீடு மட்டுமே",
  "More from the market board": "சந்தை பலகையில் இருந்து மேலும்",
  "You may also need": "உங்களுக்கு இவையும் தேவைப்படலாம்",
  "Similar quality, nearby harvests, and complementary produce.":
    "ஒத்த தரம், அருகிலுள்ள அறுவடைகள், துணை விளைபொருட்கள்.",
  Placed: "பதிவு",
  "Confirmed by farmer": "விவசாயி உறுதி",
  "Harvest & pack": "அறுவடை & பொதி",
  "Handed to transporter": "போக்குவரத்தாளரிடம் ஒப்படைப்பு",
  "In transit": "பயணத்தில்",
  Delivered: "விநியோகிக்கப்பட்டது",
  Completed: "நிறைவு",
  "Awaiting update": "புதுப்பிப்புக்காக காத்திருப்பு",
  "Produce variety": "விளைபொருள் வகை",
  Unit: "அலகு",
  Grade: "தரம்",
  "Available from": "கிடைக்கும் (முதல்)",
  "Available until": "கிடைக்கும் (வரை)",
  "Expected price / kg": "எதிர்பார்க்கும் விலை / கிலோ",
  "Publish listing": "பட்டியலை வெளியிடு",
  "Quantity must be greater than zero.":
    "அளவு பூஜ்ஜியத்தை விட அதிகமாக இருக்க வேண்டும்.",
  "Available-until must be after available-from and cannot be in the past.":
    "கிடைக்கும்-வரை தேதி கிடைக்கும்-முதல் தேதிக்குப் பிறகு இருக்க வேண்டும், கடந்த காலத்தில் இருக்கக்கூடாது.",
  "Regional suggested price: ₹30/kg": "பிராந்திய பரிந்துரை விலை: ₹30/கிலோ",
  "Choose language": "மொழியைத் தேர்ந்தெடு",
  Cart: "கூடை",
  "Your produce is being packed": "உங்கள் விளைபொருள் பொதியப்படுகிறது",
  "Collection expected 14:30–15:15": "சேகரிப்பு எதிர்பார்ப்பு 14:30–15:15",
  "A transporter will be assigned after the farmer marks your order ready.":
    "விவசாயி உங்கள் ஆர்டரைத் தயார் எனக் குறித்த பிறகு போக்குவரத்தாளர் நியமிக்கப்படுவார்.",
  "DELIVERY ADDRESS": "விநியோக முகவரி",
  "Route shown is an estimate, not live GPS.":
    "காட்டப்படும் வழி மதிப்பீடு மட்டுமே, நேரடி GPS அல்ல.",
  "Publish harvest availability": "அறுவடை கிடைப்பை வெளியிடு",
  "FARMER DESK / NEW LISTING": "விவசாயி மேசை / புதிய பட்டியல்",
  "Farm location: Chevella, Telangana. Complete your profile before publishing if this is not correct.":
    "பண்ணை இடம்: செவெல்லா, தெலுங்கானா. இது சரியில்லையெனில் வெளியிடும் முன் சுயவிவரத்தை நிறைவு செய்க.",
  "Agriculture field notes": "வேளாண் குறிப்புகள்",
  "Practical market, crop, and weather guidance for producers and buyers.":
    "உற்பத்தியாளர்கள், வாங்குபவர்களுக்கான சந்தை, பயிர், வானிலை நடைமுறை வழிகாட்டுதல்.",
  "{count} live listings · data updated 8 min ago":
    "{count} நேரடி பட்டியல்கள் · 8 நிமிடங்களுக்கு முன் புதுப்பிக்கப்பட்டது",
  "No produce matches those filters.":
    "இந்த வடிப்பான்களுக்கு எந்த விளைபொருளும் பொருந்தவில்லை.",
  "Broaden your location, price, or freshness filter to see live farm stock.":
    "நேரடி பண்ணை இருப்பைக் காண இடம், விலை அல்லது புத்துணர்வு வடிப்பானை விரிவாக்குங்கள்.",
  Delete: "நீக்கு",
  "Delete this listing permanently? This cannot be undone.":
    "இந்தப் பட்டியலை நிரந்தரமாக நீக்கவா? இதைச் செயல்தவிர்க்க முடியாது.",
  "No listings yet. Publish your first harvest to appear here.":
    "இன்னும் பட்டியல்கள் இல்லை. இங்கே தோன்ற உங்கள் முதல் அறுவடையை வெளியிடுங்கள்.",
  "Price must be greater than zero.":
    "விலை பூஜ்ஜியத்தை விட அதிகமாக இருக்க வேண்டும்.",
  Yours: "உங்களுடையது",
  Category: "வகை",
  "Clear cart": "கூடையை காலி செய்",
  "Continue shopping": "தொடர்ந்து வாங்குங்கள்",
  "{count} items": "{count} உருப்படிகள்",
  "Delivery slot": "விநியோக நேரம்",
  "Decrease quantity": "அளவைக் குறை",
  "Increase quantity": "அளவை அதிகரி",
  "Remove from cart": "கூடையிலிருந்து நீக்கு",
  "Payment did not complete. Your order is still placed; retry safely.":
    "கட்டணம் நிறைவடையவில்லை. உங்கள் ஆர்டர் அப்படியே உள்ளது; பாதுகாப்பாக மீண்டும் முயலுங்கள்.",
  Analytics: "பகுப்பாய்வு",
  "View analytics": "பகுப்பாய்வைக் காண்க",
  "Stock value overview": "இருப்பு மதிப்பு கண்ணோட்டம்",
  "Live market data across every visible listing.":
    "ஒவ்வொரு பட்டியலின் நேரடி சந்தை தரவு.",
  "Total stock value": "மொத்த இருப்பு மதிப்பு",
  "Paddy stock value": "நெல் இருப்பு மதிப்பு",
  "Vegetables stock value": "காய்கறி இருப்பு மதிப்பு",
  "Share of total stock": "மொத்த இருப்பில் பங்கு",
  "Paddy vs vegetables": "நெல் vs காய்கறிகள்",
  "Other produce": "மற்ற விளைபொருட்கள்",
  "Stock value by category": "வகை வாரியாக இருப்பு மதிப்பு",
  "Top listings by stock value": "இருப்பு மதிப்பில் முதல் பட்டியல்கள்",
  Value: "மதிப்பு",
  "AI assistant": "AI உதவியாளர்",
  "Ask about prices, delivery, orders…":
    "விலை, விநியோகம், ஆர்டர்கள் பற்றி கேளுங்கள்…",
  "Online · replies instantly": "ஆன்லைன் · உடனடி பதில்",
  "Paddy prices": "நெல் விலைகள்",
  "How to sell": "எப்படி விற்பது",
  "Report this listing": "இந்தப் பட்டியலைப் புகாரளிக்கவும்",
  Reported: "புகாரளிக்கப்பட்டது",
  "Why are you reporting this listing?":
    "இந்தப் பட்டியலை ஏன் புகாரளிக்கிறீர்கள்?",
  "Wrong price": "தவறான விலை",
  "Poor quality": "மோசமான தரம்",
  "Sold out": "இருப்பு தீர்ந்துவிட்டது",
  "Spam or fraud": "ஸ்பேம் அல்லது மோசடி",
  "Tell us more (optional)": "மேலும் கூறுங்கள் (விருப்பம்)",
  Cancel: "ரத்து செய்",
  "Submit report": "புகாரைச் சமர்ப்பி",
  "Thanks — our team will review this listing.":
    "நன்றி — எங்கள் குழு இந்தப் பட்டியலை மதிப்பாய்வு செய்யும்.",
};

const kn: Dict = {
  "Buy produce": "ಬೆಳೆ ಖರೀದಿಸಿ",
  "Sell produce": "ಬೆಳೆ ಮಾರಿ",
  Learn: "ಕಲಿಯಿರಿ",
  "Direct produce exchange · India": "ನೇರ ಬೆಳೆ ವಿನಿಮಯ · ಭಾರತ",
  "Better prices start at the farm gate.":
    "ಉತ್ತಮ ಬೆಲೆಗಳು ಜಮೀನಿನ ಬಾಗಿಲಲ್ಲೇ ಆರಂಭವಾಗುತ್ತವೆ.",
  "AgriLink gives farmers control of inventory and buyers a clear view of fresh stock, delivery cost, and the people behind every order.":
    "AgriLink ರೈತರಿಗೆ ದಾಸ್ತಾನು ನಿಯಂತ್ರಣ ನೀಡುತ್ತದೆ, ಖರೀದಿದಾರರಿಗೆ ತಾಜಾ ಸ್ಟಾಕ್, ವಿತರಣಾ ವೆಚ್ಚ, ಪ್ರತಿ ಆರ್ಡರ್ ಹಿಂದಿನ ಜನರ ಸ್ಪಷ್ಟ ಮಾಹಿತಿ ನೀಡುತ್ತದೆ.",
  "Browse today's harvest": "ಇಂದಿನ ಬೆಳೆಯನ್ನು ನೋಡಿ",
  "List your produce": "ನಿಮ್ಮ ಬೆಳೆಯನ್ನು ಪಟ್ಟಿ ಮಾಡಿ",
  "active farm lots": "ಸಕ್ರಿಯ ಫಾರ್ಮ್ ಲಾಟ್‌ಗಳು",
  "farmer payouts this week": "ಈ ವಾರದ ರೈತ ಪಾವತಿಗಳು",
  "MARKET UPDATE": "ಮಾರುಕಟ್ಟೆ ಅಪ್‌ಡೇಟ್",
  "Fresh lots added all day": "ದಿನವಿಡೀ ತಾಜಾ ಲಾಟ್‌ಗಳು ಸೇರುತ್ತವೆ",
  "Compare quality, source, and price before buying":
    "ಖರೀದಿಸುವ ಮೊದಲು ಗುಣಮಟ್ಟ, ಮೂಲ, ಬೆಲೆಯನ್ನು ಹೋಲಿಸಿ",
  "Fresh off the field": "ಹೊಲದಿಂದ ತಾಜಾ",
  "Market board": "ಮಾರುಕಟ್ಟೆ ಮಂಡಳಿ",
  "See all listings →": "ಎಲ್ಲಾ ಪಟ್ಟಿಗಳನ್ನು ನೋಡಿ →",
  "Price clarity": "ಬೆಲೆ ಸ್ಪಷ್ಟತೆ",
  "See regional price baselines and your net payout before you accept.":
    "ಒಪ್ಪುವ ಮೊದಲು ಪ್ರಾದೇಶಿಕ ಬೆಲೆ ಮಾನದಂಡಗಳು, ನಿಮ್ಮ ನಿವ್ವಳ ಪಾವತಿಯನ್ನು ನೋಡಿ.",
  "Coordinated delivery": "ಸಂಯೋಜಿತ ವಿತರಣೆ",
  "Transparent distance and multi-stop route estimates for every order.":
    "ಪ್ರತಿ ಆರ್ಡರ್‌ಗೆ ಪಾರದರ್ಶಕ ದೂರ, ಬಹು-ನಿಲ್ದಾಣ ಮಾರ್ಗ ಅಂದಾಜುಗಳು.",
  "Tracked lifecycle": "ಟ್ರ್ಯಾಕ್ ಮಾಡಿದ ಜೀವನಚಕ್ರ",
  "From harvest pack to hand-off to delivery, every update is visible.":
    "ಕೊಯ್ಲು-ಪ್ಯಾಕಿಂಗ್‌ನಿಂದ ಹಸ್ತಾಂತರದಿಂದ ವಿತರಣೆಯವರೆಗೆ ಪ್ರತಿ ಅಪ್‌ಡೇಟ್ ಕಾಣುತ್ತದೆ.",
  "Live farm inventory · Hyderabad region":
    "ಲೈವ್ ಫಾರ್ಮ್ ದಾಸ್ತಾನು · ಹೈದರಾಬಾದ್ ಪ್ರದೇಶ",
  "Buy closer to the source.": "ಮೂಲಕ್ಕೆ ಹತ್ತಿರದಿಂದ ಖರೀದಿಸಿ.",
  "Try: fresh tomatoes under ₹30/kg near Hyderabad":
    "ಪ್ರಯತ್ನಿಸಿ: ಹೈದರಾಬಾದ್ ಬಳಿ ₹30/ಕಿಲೋ ಒಳಗೆ ತಾಜಾ ಟೊಮೆಟೊ",
  "Best match": "ಉತ್ತಮ ಹೊಂದಾಣಿಕೆ",
  "Price: low to high": "ಬೆಲೆ: ಕಡಿಮೆಯಿಂದ ಹೆಚ್ಚು",
  Distance: "ದೂರ",
  "More filters": "ಇನ್ನಷ್ಟು ಫಿಲ್ಟರ್‌ಗಳು",
  "All produce": "ಎಲ್ಲಾ ಬೆಳೆಗಳು",
  Vegetables: "ತರಕಾರಿಗಳು",
  Fruits: "ಹಣ್ಣುಗಳು",
  Grains: "ಧಾನ್ಯಗಳು",
  Pulses: "ಬೇಳೆಗಳು",
  Spices: "ಮಸಾಲೆಗಳು",
  Rice: "ಅಕ್ಕಿ",
  "Contact number": "ಸಂಪರ್ಕ ಸಂಖ್ಯೆ",
  "Seller phone · bulk orders": "ಮಾರಾಟಗಾರ ಫೋನ್ · ಸಗಟು ಆರ್ಡರ್ಗಳು",
  "Call seller for bulk orders": "ಸಗಟು ಆರ್ಡರ್ಗಳಿಗಾಗಿ ಮಾರಾಟಗಾರರಿಗೆ ಕರೆ ಮಾಡಿ",
  Harvested: "ಕೊಯ್ಲು",
  "km away": "ಕಿಮೀ ದೂರ",
  "Your collection cart": "ನಿಮ್ಮ ಸಂಗ್ರಹ ಬುಟ್ಟಿ",
  "Adjust quantities, remove items, or continue to a final server-side stock check.":
    "ಪ್ರಮಾಣ ಬದಲಿಸಿ, ವಸ್ತುಗಳನ್ನು ತೆಗೆದುಹಾಕಿ ಅಥವಾ ಅಂತಿಮ ಸ್ಟಾಕ್ ಪರಿಶೀಲನೆಗೆ ಮುಂದುವರಿಯಿರಿ.",
  "Your cart is empty.": "ನಿಮ್ಮ ಬುಟ್ಟಿ ಖಾಲಿಯಿದೆ.",
  "Pick freshly listed produce from the market board.":
    "ಮಾರುಕಟ್ಟೆ ಮಂಡಳಿಯಿಂದ ತಾಜಾ ಬೆಳೆಯನ್ನು ಆರಿಸಿ.",
  "Browse marketplace": "ಮಾರುಕಟ್ಟೆ ನೋಡಿ",
  "Order summary": "ಆರ್ಡರ್ ಸಾರಾಂಶ",
  Produce: "ಬೆಳೆ",
  "Delivery estimate": "ವಿತರಣಾ ಅಂದಾಜು",
  "Platform fee": "ಪ್ಲಾಟ್‌ಫಾರ್ಮ್ ಶುಲ್ಕ",
  Total: "ಒಟ್ಟು",
  "Continue to checkout": "ಚೆಕ್‌ಔಟ್‌ಗೆ ಹೋಗಿ",
  "Checkout · secure demo flow": "ಚೆಕ್‌ಔಟ್ · ಸುರಕ್ಷಿತ ಡೆಮೊ ಪ್ರಕ್ರಿಯೆ",
  "Confirm collection & delivery": "ಸಂಗ್ರಹ & ವಿತರಣೆಯನ್ನು ದೃಢೀಕರಿಸಿ",
  "Delivery details": "ವಿತರಣಾ ವಿವರಗಳು",
  "Tomorrow · 9–12": "ನಾಳೆ · 9–12",
  "Tomorrow · 2–5": "ನಾಳೆ · 2–5",
  Payment: "ಪಾವತಿ",
  "Demo checkout — no payment details are stored.":
    "ಡೆಮೊ ಚೆಕ್‌ಔಟ್ — ಪಾವತಿ ವಿವರಗಳನ್ನು ಸಂಗ್ರಹಿಸಲಾಗುವುದಿಲ್ಲ.",
  "Order placed successfully.": "ಆರ್ಡರ್ ಯಶಸ್ವಿಯಾಗಿ ನೀಡಲಾಗಿದೆ.",
  "Payment record AGRPAY-90217 saved. Farmer confirmation is next.":
    "ಪಾವತಿ ದಾಖಲೆ AGRPAY-90217 ಉಳಿಸಲಾಗಿದೆ. ಮುಂದೆ ರೈತರ ದೃಢೀಕರಣ.",
  "Track order": "ಆರ್ಡರ್ ಟ್ರ್ಯಾಕ್ ಮಾಡಿ",
  "Cash collection is pending on delivery. Your order can proceed.":
    "ವಿತರಣೆಯ ಮೇಲೆ ನಗದು ಸಂಗ್ರಹ ಬಾಕಿ ಇದೆ. ನಿಮ್ಮ ಆರ್ಡರ್ ಮುಂದುವರಿಯಬಹುದು.",
  "Simulate payment": "ಪಾವತಿಯನ್ನು ಅನುಕರಿಸಿ",
  "Place order": "ಆರ್ಡರ್ ನೀಡಿ",
  "SELL PRODUCE": "ಬೆಳೆ ಮಾರಿ",
  "Your selling board": "ನಿಮ್ಮ ಮಾರಾಟ ಮಂಡಳಿ",
  "Any AgriLink member can list fresh produce and manage their own sales.":
    "ಯಾವುದೇ AgriLink ಸದಸ್ಯರು ತಾಜಾ ಬೆಳೆಯನ್ನು ಪಟ್ಟಿ ಮಾಡಿ ಸ್ವಂತ ಮಾರಾಟ ನಿರ್ವಹಿಸಬಹುದು.",
  "New listing": "ಹೊಸ ಪಟ್ಟಿ",
  "Active listings": "ಸಕ್ರಿಯ ಪಟ್ಟಿಗಳು",
  "Est. earnings": "ಅಂದಾಜು ಗಳಿಕೆ",
  "Orders in progress": "ಪ್ರಗತಿಯಲ್ಲಿರುವ ಆರ್ಡರ್‌ಗಳು",
  "Stock running low": "ಸ್ಟಾಕ್ ಕಡಿಮೆಯಾಗುತ್ತಿದೆ",
  "Your active stock": "ನಿಮ್ಮ ಸಕ್ರಿಯ ಸ್ಟಾಕ್",
  "Manage stock": "ಸ್ಟಾಕ್ ನಿರ್ವಹಿಸಿ",
  "Price advisory": "ಬೆಲೆ ಸಲಹೆ",
  "Tomato market position": "ಟೊಮೆಟೊ ಮಾರುಕಟ್ಟೆ ಸ್ಥಿತಿ",
  "your listing / kg": "ನಿಮ್ಮ ಪಟ್ಟಿ / ಕಿಲೋ",
  "Set a location for nearby prices and delivery estimates":
    "ಹತ್ತಿರದ ಬೆಲೆಗಳು, ವಿತರಣಾ ಅಂದಾಜುಗಳಿಗಾಗಿ ಸ್ಥಳವನ್ನು ಹೊಂದಿಸಿ",
  "You can skip this and explore every market price first.":
    "ಇದನ್ನು ಬಿಟ್ಟು ಮೊದಲು ಎಲ್ಲಾ ಮಾರುಕಟ್ಟೆ ಬೆಲೆಗಳನ್ನು ನೋಡಬಹುದು.",
  "Set location": "ಸ್ಥಳ ಹೊಂದಿಸಿ",
  "Not now": "ಈಗ ಬೇಡ",
  "Save location": "ಸ್ಥಳ ಉಳಿಸಿ",
  "Village, district, state, or pincode": "ಗ್ರಾಮ, ಜಿಲ್ಲೆ, ರಾಜ್ಯ ಅಥವಾ ಪಿನ್‌ಕೋಡ್",
  "Sign in": "ಸೈನ್ ಇನ್",
  "Log out": "ಲಾಗ್ ಔಟ್",
  "AGRLINK ACCESS": "ಎಗ್ರಿಲಿಂಕ್ ಪ್ರವೇಶ",
  "Sign in to your desk": "ನಿಮ್ಮ ಡೆಸ್ಕ್‌ಗೆ ಸೈನ್ ಇನ್ ಮಾಡಿ",
  "New to AgriLink?": "AgriLinkಗೆ ಹೊಸಬರೇ?",
  "Create an account": "ಖಾತೆ ರಚಿಸಿ",
  "CREATE ACCOUNT": "ಖಾತೆ ರಚಿಸಿ",
  "Join the open market": "ಮುಕ್ತ ಮಾರುಕಟ್ಟೆಗೆ ಸೇರಿ",
  "Full name": "ಪೂರ್ಣ ಹೆಸರು",
  "Email address": "ಇಮೇಲ್ ವಿಳಾಸ",
  "Mobile number": "ಮೊಬೈಲ್ ಸಂಖ್ಯೆ",
  Password: "ಪಾಸ್‌ವರ್ಡ್",
  "Every account can buy and sell. Add a location when you are ready to publish produce or set delivery preferences.":
    "ಪ್ರತಿ ಖಾತೆಯೂ ಖರೀದಿಸಬಹುದು, ಮಾರಬಹುದು. ಬೆಳೆ ಪ್ರಕಟಿಸಲು ಅಥವಾ ವಿತರಣಾ ಆದ್ಯತೆ ಹೊಂದಿಸಲು ಸಿದ್ಧವಾದಾಗ ಸ್ಥಳ ಸೇರಿಸಿ.",
  "Create account": "ಖಾತೆ ರಚಿಸಿ",
  "Knowledge hub": "ಜ್ಞಾನ ಕೇಂದ್ರ",
  QUALITY: "ಗುಣಮಟ್ಟ",
  HARVEST: "ಕೊಯ್ಲು",
  STOCK: "ಸ್ಟಾಕ್",
  per: "ಗೆ",
  Quantity: "ಪ್ರಮಾಣ",
  "Add to cart": "ಬುಟ್ಟಿಗೆ ಸೇರಿಸಿ",
  "Added to cart": "ಬುಟ್ಟಿಗೆ ಸೇರಿಸಲಾಗಿದೆ",
  Inquiry: "ವಿಚಾರಣೆ",
  "Minimum bulk order: 20 kg. Stock is verified again at checkout to protect against orders placed by other buyers.":
    "ಕನಿಷ್ಠ ಸಗಟು ಆರ್ಡರ್: 20 ಕೆಜಿ. ಇತರ ಖರೀದಿದಾರರ ಆರ್ಡರ್‌ಗಳಿಂದ ರಕ್ಷಣೆಗೆ ಚೆಕ್‌ಔಟ್‌ನಲ್ಲಿ ಸ್ಟಾಕ್ ಮತ್ತೆ ಪರಿಶೀಲಿಸಲಾಗುತ್ತದೆ.",
  "Platform-coordinated delivery": "ಪ್ಲಾಟ್‌ಫಾರ್ಮ್-ಸಂಯೋಜಿತ ವಿತರಣೆ",
  "route estimate only": "ಮಾರ್ಗ ಅಂದಾಜು ಮಾತ್ರ",
  "More from the market board": "ಮಾರುಕಟ್ಟೆ ಮಂಡಳಿಯಿಂದ ಇನ್ನಷ್ಟು",
  "You may also need": "ನಿಮಗೆ ಇವುಗಳೂ ಬೇಕಾಗಬಹುದು",
  "Similar quality, nearby harvests, and complementary produce.":
    "ಸಮಾನ ಗುಣಮಟ್ಟ, ಹತ್ತಿರದ ಕೊಯ್ಲುಗಳು, ಪೂರಕ ಬೆಳೆಗಳು.",
  Placed: "ದಾಖಲು",
  "Confirmed by farmer": "ರೈತರಿಂದ ದೃಢೀಕೃತ",
  "Harvest & pack": "ಕೊಯ್ಲು & ಪ್ಯಾಕಿಂಗ್",
  "Handed to transporter": "ಸಾಗಣೆದಾರರಿಗೆ ಹಸ್ತಾಂತರ",
  "In transit": "ಸಾಗಣೆಯಲ್ಲಿ",
  Delivered: "ವಿತರಿಸಲಾಗಿದೆ",
  Completed: "ಪೂರ್ಣ",
  "Awaiting update": "ಅಪ್‌ಡೇಟ್‌ಗಾಗಿ ಕಾಯುವಿಕೆ",
  "Produce variety": "ಬೆಳೆ ತಳಿ",
  Unit: "ಘಟಕ",
  Grade: "ಗ್ರೇಡ್",
  "Available from": "ಲಭ್ಯ (ಇಂದ)",
  "Available until": "ಲಭ್ಯ (ವರೆಗೆ)",
  "Expected price / kg": "ನಿರೀಕ್ಷಿತ ಬೆಲೆ / ಕಿಲೋ",
  "Publish listing": "ಪಟ್ಟಿ ಪ್ರಕಟಿಸಿ",
  "Quantity must be greater than zero.": "ಪ್ರಮಾಣ ಶೂನ್ಯಕ್ಕಿಂತ ಹೆಚ್ಚಿರಬೇಕು.",
  "Available-until must be after available-from and cannot be in the past.":
    "ಲಭ್ಯ-ವರೆಗೆ ದಿನಾಂಕ ಲಭ್ಯ-ಇಂದ ನಂತರ ಇರಬೇಕು, ಹಿಂದಿನದಾಗಿರಬಾರದು.",
  "Regional suggested price: ₹30/kg": "ಪ್ರಾದೇಶಿಕ ಸೂಚಿತ ಬೆಲೆ: ₹30/ಕಿಲೋ",
  "Choose language": "ಭಾಷೆ ಆರಿಸಿ",
  Cart: "ಬುಟ್ಟಿ",
  "Your produce is being packed": "ನಿಮ್ಮ ಬೆಳೆಯನ್ನು ಪ್ಯಾಕ್ ಮಾಡಲಾಗುತ್ತಿದೆ",
  "Collection expected 14:30–15:15": "ಸಂಗ್ರಹ ನಿರೀಕ್ಷೆ 14:30–15:15",
  "A transporter will be assigned after the farmer marks your order ready.":
    "ರೈತರು ನಿಮ್ಮ ಆರ್ಡರ್ ಸಿದ್ಧ ಎಂದು ಗುರುತಿಸಿದ ನಂತರ ಸಾಗಣೆದಾರರನ್ನು ನಿಯೋಜಿಸಲಾಗುತ್ತದೆ.",
  "DELIVERY ADDRESS": "ವಿತರಣಾ ವಿಳಾಸ",
  "Route shown is an estimate, not live GPS.":
    "ತೋರಿಸಿದ ಮಾರ್ಗ ಅಂದಾಜು ಮಾತ್ರ, ಲೈವ್ GPS ಅಲ್ಲ.",
  "Publish harvest availability": "ಬೆಳೆ ಲಭ್ಯತೆ ಪ್ರಕಟಿಸಿ",
  "FARMER DESK / NEW LISTING": "ರೈತ ಡೆಸ್ಕ್ / ಹೊಸ ಪಟ್ಟಿ",
  "Farm location: Chevella, Telangana. Complete your profile before publishing if this is not correct.":
    "ಜಮೀನು ಸ್ಥಳ: ಚೆವೆಲ್ಲಾ, ತೆಲಂಗಾಣ. ಇದು ಸರಿಯಿಲ್ಲದಿದ್ದರೆ ಪ್ರಕಟಿಸುವ ಮೊದಲು ಪ್ರೊಫೈಲ್ ಪೂರ್ಣಗೊಳಿಸಿ.",
  "Agriculture field notes": "ಕೃಷಿ ಟಿಪ್ಪಣಿಗಳು",
  "Practical market, crop, and weather guidance for producers and buyers.":
    "ಉತ್ಪಾದಕರು, ಖರೀದಿದಾರರಿಗೆ ಮಾರುಕಟ್ಟೆ, ಬೆಳೆ, ಹವಾಮಾನ ಪ್ರಾಯೋಗಿಕ ಮಾರ್ಗದರ್ಶನ.",
  "{count} live listings · data updated 8 min ago":
    "{count} ಲೈವ್ ಪಟ್ಟಿಗಳು · 8 ನಿಮಿಷಗಳ ಹಿಂದೆ ನವೀಕರಿಸಲಾಗಿದೆ",
  "No produce matches those filters.":
    "ಈ ಫಿಲ್ಟರ್‌ಗಳಿಗೆ ಯಾವುದೇ ಬೆಳೆ ಹೊಂದಿಕೆಯಾಗಲಿಲ್ಲ.",
  "Broaden your location, price, or freshness filter to see live farm stock.":
    "ಲೈವ್ ಫಾರ್ಮ್ ಸ್ಟಾಕ್ ನೋಡಲು ಸ್ಥಳ, ಬೆಲೆ ಅಥವಾ ತಾಜಾತನ ಫಿಲ್ಟರ್ ವಿಸ್ತರಿಸಿ.",
  Delete: "ಅಳಿಸಿ",
  "Delete this listing permanently? This cannot be undone.":
    "ಈ ಪಟ್ಟಿಯನ್ನು ಶಾಶ್ವತವಾಗಿ ಅಳಿಸಬೇಕೇ? ಇದನ್ನು ರದ್ದುಗೊಳಿಸಲು ಸಾಧ್ಯವಿಲ್ಲ.",
  "No listings yet. Publish your first harvest to appear here.":
    "ಇನ್ನೂ ಯಾವುದೇ ಪಟ್ಟಿಗಳಿಲ್ಲ. ಇಲ್ಲಿ ಕಾಣಿಸಿಕೊಳ್ಳಲು ನಿಮ್ಮ ಮೊದಲ ಬೆಳೆಯನ್ನು ಪ್ರಕಟಿಸಿ.",
  "Price must be greater than zero.": "ಬೆಲೆ ಶೂನ್ಯಕ್ಕಿಂತ ಹೆಚ್ಚಿರಬೇಕು.",
  Yours: "ನಿಮ್ಮದು",
  Category: "ವರ್ಗ",
  "Clear cart": "ಬುಟ್ಟಿಯನ್ನು ಖಾಲಿ ಮಾಡಿ",
  "Continue shopping": "ಖರೀದಿಯನ್ನು ಮುಂದುವರಿಸಿ",
  "{count} items": "{count} ವಸ್ತುಗಳು",
  "Delivery slot": "ವಿತರಣಾ ಸಮಯ",
  "Decrease quantity": "ಪ್ರಮಾಣ ಕಡಿಮೆ ಮಾಡಿ",
  "Increase quantity": "ಪ್ರಮಾಣ ಹೆಚ್ಚಿಸಿ",
  "Remove from cart": "ಬುಟ್ಟಿಯಿಂದ ತೆಗೆದುಹಾಕಿ",
  "Payment did not complete. Your order is still placed; retry safely.":
    "ಪಾವತಿ ಪೂರ್ಣಗೊಳ್ಳಲಿಲ್ಲ. ನಿಮ್ಮ ಆರ್ಡರ್ ಹಾಗೆಯೇ ಇದೆ; ಸುರಕ್ಷಿತವಾಗಿ ಮತ್ತೆ ಪ್ರಯತ್ನಿಸಿ.",
  Analytics: "ವಿಶ್ಲೇಷಣೆ",
  "View analytics": "ವಿಶ್ಲೇಷಣೆ ನೋಡಿ",
  "Stock value overview": "ದಾಸ್ತಾನು ಮೌಲ್ಯ ಅವಲೋಕನ",
  "Live market data across every visible listing.":
    "ಪ್ರತಿ ಗೋಚರ ಪಟ್ಟಿಯ ಲೈವ್ ಮಾರುಕಟ್ಟೆ ಡೇಟಾ.",
  "Total stock value": "ಒಟ್ಟು ದಾಸ್ತಾನು ಮೌಲ್ಯ",
  "Paddy stock value": "ಭತ್ತದ ದಾಸ್ತಾನು ಮೌಲ್ಯ",
  "Vegetables stock value": "ತರಕಾರಿ ದಾಸ್ತಾನು ಮೌಲ್ಯ",
  "Share of total stock": "ಒಟ್ಟು ದಾಸ್ತಾನಿನಲ್ಲಿ ಪಾಲು",
  "Paddy vs vegetables": "ಭತ್ತ vs ತರಕಾರಿ",
  "Other produce": "ಇತರೆ ಬೆಳೆ",
  "Stock value by category": "ವರ್ಗವಾರು ದಾಸ್ತಾನು ಮೌಲ್ಯ",
  "Top listings by stock value": "ದಾಸ್ತಾನು ಮೌಲ್ಯದಲ್ಲಿ ಅಗ್ರ ಪಟ್ಟಿಗಳು",
  Value: "ಮೌಲ್ಯ",
  "AI assistant": "AI ಸಹಾಯಕ",
  "Ask about prices, delivery, orders…": "ಬೆಲೆ, ವಿತರಣೆ, ಆರ್ಡರ್‌ಗಳ ಬಗ್ಗೆ ಕೇಳಿ…",
  "Online · replies instantly": "ಆನ್‌ಲೈನ್ · ತಕ್ಷಣದ ಉತ್ತರ",
  "Paddy prices": "ಭತ್ತದ ಬೆಲೆಗಳು",
  "How to sell": "ಹೇಗೆ ಮಾರುವುದು",
  "Report this listing": "ಈ ಪಟ್ಟಿಯನ್ನು ವರದಿ ಮಾಡಿ",
  Reported: "ವರದಿ ಮಾಡಲಾಗಿದೆ",
  "Why are you reporting this listing?":
    "ನೀವು ಈ ಪಟ್ಟಿಯನ್ನು ಏಕೆ ವರದಿ ಮಾಡುತ್ತಿದ್ದೀರಿ?",
  "Wrong price": "ತಪ್ಪು ಬೆಲೆ",
  "Poor quality": "ಕಳಪೆ ಗುಣಮಟ್ಟ",
  "Sold out": "ಸ್ಟಾಕ್ ಮುಗಿದಿದೆ",
  "Spam or fraud": "ಸ್ಪ್ಯಾಮ್ ಅಥವಾ ವಂಚನೆ",
  "Tell us more (optional)": "ಇನ್ನಷ್ಟು ತಿಳಿಸಿ (ಐಚ್ಛಿಕ)",
  Cancel: "ರದ್ದುಮಾಡಿ",
  "Submit report": "ವರದಿಯನ್ನು ಸಲ್ಲಿಸಿ",
  "Thanks — our team will review this listing.":
    "ಧನ್ಯವಾದಗಳು — ನಮ್ಮ ತಂಡ ಈ ಪಟ್ಟಿಯನ್ನು ಪರಿಶೀಲಿಸುತ್ತದೆ.",
};

const mr: Dict = {
  "Buy produce": "शेतमाल खरेदी करा",
  "Sell produce": "शेतमाल विका",
  Learn: "शिका",
  "Direct produce exchange · India": "थेट शेतमाल विनिमय · भारत",
  "Better prices start at the farm gate.":
    "चांगले भाव शेताच्या दारातूनच सुरू होतात.",
  "AgriLink gives farmers control of inventory and buyers a clear view of fresh stock, delivery cost, and the people behind every order.":
    "AgriLink शेतकऱ्यांना साठ्यावर नियंत्रण देते आणि खरेदीदारांना ताजा साठा, वितरण खर्च व प्रत्येक ऑर्डरमागील लोकांची स्पष्ट माहिती देते.",
  "Browse today's harvest": "आजचा शेतमाल पहा",
  "List your produce": "तुमचा शेतमाल सूचीबद्ध करा",
  "active farm lots": "सक्रिय फार्म लॉट",
  "farmer payouts this week": "या आठवड्यातील शेतकरी देयके",
  "MARKET UPDATE": "बाजार अपडेट",
  "Fresh lots added all day": "दिवसभर ताजे लॉट जोडले जातात",
  "Compare quality, source, and price before buying":
    "खरेदीपूर्वी गुणवत्ता, स्रोत आणि किंमत तुलना करा",
  "Fresh off the field": "शेतातून ताजे",
  "Market board": "बाजार फलक",
  "See all listings →": "सर्व सूची पहा →",
  "Price clarity": "किंमत स्पष्टता",
  "See regional price baselines and your net payout before you accept.":
    "स्वीकारण्यापूर्वी प्रादेशिक किंमत आधार आणि तुमचे निव्वळ देयक पहा.",
  "Coordinated delivery": "समन्वित वितरण",
  "Transparent distance and multi-stop route estimates for every order.":
    "प्रत्येक ऑर्डरसाठी पारदर्शक अंतर आणि मल्टी-स्टॉप मार्ग अंदाज.",
  "Tracked lifecycle": "ट्रॅक केलेले जीवनचक्र",
  "From harvest pack to hand-off to delivery, every update is visible.":
    "काढणी-पॅकिंगपासून हस्तांतरापासून वितरणापर्यंत प्रत्येक अपडेट दिसते.",
  "Live farm inventory · Hyderabad region":
    "लाइव्ह फार्म साठा · हैदराबाद प्रदेश",
  "Buy closer to the source.": "उगमाच्या जवळून खरेदी करा.",
  "Try: fresh tomatoes under ₹30/kg near Hyderabad":
    "करून पहा: हैदराबादजवळ ₹30/किलोखालील ताजे टोमॅटो",
  "Best match": "सर्वोत्तम जुळणी",
  "Price: low to high": "किंमत: कमी ते जास्त",
  Distance: "अंतर",
  "More filters": "अधिक फिल्टर",
  "All produce": "सर्व शेतमाल",
  Vegetables: "भाज्या",
  Fruits: "फळे",
  Grains: "धान्य",
  Pulses: "डाळी",
  Spices: "मसाले",
  Rice: "तांदूळ",
  "Contact number": "संपर्क क्रमांक",
  "Seller phone · bulk orders": "विक्रेता फोन · मोठ्या ऑर्डरसाठी",
  "Call seller for bulk orders": "मोठ्या ऑर्डरसाठी विक्रेत्याला कॉल करा",
  Harvested: "काढणी",
  "km away": "किमी दूर",
  "Your collection cart": "तुमची संकलन टोपली",
  "Adjust quantities, remove items, or continue to a final server-side stock check.":
    "प्रमाण बदला, वस्तू काढा किंवा अंतिम साठा तपासणीकडे जा.",
  "Your cart is empty.": "तुमची टोपली रिकामी आहे.",
  "Pick freshly listed produce from the market board.":
    "बाजार फलकावरील ताजा शेतमाल निवडा.",
  "Browse marketplace": "बाजार पहा",
  "Order summary": "ऑर्डर सारांश",
  Produce: "शेतमाल",
  "Delivery estimate": "वितरण अंदाज",
  "Platform fee": "प्लॅटफॉर्म शुल्क",
  Total: "एकूण",
  "Continue to checkout": "चेकआउटवर जा",
  "Checkout · secure demo flow": "चेकआउट · सुरक्षित डेमो प्रक्रिया",
  "Confirm collection & delivery": "संकलन व वितरणाची पुष्टी करा",
  "Delivery details": "वितरण तपशील",
  "Tomorrow · 9–12": "उद्या · 9–12",
  "Tomorrow · 2–5": "उद्या · 2–5",
  Payment: "पेमेंट",
  "Demo checkout — no payment details are stored.":
    "डेमो चेकआउट — पेमेंट तपशील साठवले जात नाहीत.",
  "Order placed successfully.": "ऑर्डर यशस्वीरित्या नोंदवली.",
  "Payment record AGRPAY-90217 saved. Farmer confirmation is next.":
    "पेमेंट रेकॉर्ड AGRPAY-90217 जतन केले. पुढे शेतकऱ्याची पुष्टी.",
  "Track order": "ऑर्डर ट्रॅक करा",
  "Cash collection is pending on delivery. Your order can proceed.":
    "वितरणावर रोख संकलन प्रलंबित आहे. तुमची ऑर्डर पुढे जाऊ शकते.",
  "Simulate payment": "पेमेंटचे अनुकरण करा",
  "Place order": "ऑर्डर द्या",
  "SELL PRODUCE": "शेतमाल विका",
  "Your selling board": "तुमचा विक्री फलक",
  "Any AgriLink member can list fresh produce and manage their own sales.":
    "कोणताही AgriLink सदस्य ताजा शेतमाल सूचीबद्ध करून स्वतःची विक्री सांभाळू शकतो.",
  "New listing": "नवीन सूची",
  "Active listings": "सक्रिय सूची",
  "Est. earnings": "अंदाजे कमाई",
  "Orders in progress": "प्रगतीतील ऑर्डर",
  "Stock running low": "साठा कमी होत आहे",
  "Your active stock": "तुमचा सक्रिय साठा",
  "Manage stock": "साठा व्यवस्थापित करा",
  "Price advisory": "किंमत सल्ला",
  "Tomato market position": "टोमॅटो बाजार स्थिती",
  "your listing / kg": "तुमची सूची / किलो",
  "Set a location for nearby prices and delivery estimates":
    "जवळचे भाव व वितरण अंदाजांसाठी स्थान निश्चित करा",
  "You can skip this and explore every market price first.":
    "तुम्ही हे वगळून आधी सर्व बाजारभाव पाहू शकता.",
  "Set location": "स्थान निश्चित करा",
  "Not now": "आत्ता नको",
  "Save location": "स्थान जतन करा",
  "Village, district, state, or pincode": "गाव, जिल्हा, राज्य किंवा पिनकोड",
  "Sign in": "साइन इन",
  "Log out": "लॉग आउट",
  "AGRLINK ACCESS": "एग्रीलिंक प्रवेश",
  "Sign in to your desk": "तुमच्या डेस्कवर साइन इन करा",
  "New to AgriLink?": "AgriLink वर नवीन आहात?",
  "Create an account": "खाते तयार करा",
  "CREATE ACCOUNT": "खाते तयार करा",
  "Join the open market": "खुल्या बाजारात सामील व्हा",
  "Full name": "पूर्ण नाव",
  "Email address": "ईमेल पत्ता",
  "Mobile number": "मोबाईल नंबर",
  Password: "पासवर्ड",
  "Every account can buy and sell. Add a location when you are ready to publish produce or set delivery preferences.":
    "प्रत्येक खाते खरेदी-विक्री करू शकते. शेतमाल प्रसिद्ध करण्यासाठी किंवा वितरण प्राधान्ये ठरवण्यासाठी स्थान जोडा.",
  "Create account": "खाते तयार करा",
  "Knowledge hub": "ज्ञान केंद्र",
  QUALITY: "गुणवत्ता",
  HARVEST: "काढणी",
  STOCK: "साठा",
  per: "प्रति",
  Quantity: "प्रमाण",
  "Add to cart": "टोपलीत टाका",
  "Added to cart": "टोपलीत टाकले",
  Inquiry: "चौकशी",
  "Minimum bulk order: 20 kg. Stock is verified again at checkout to protect against orders placed by other buyers.":
    "किमान घाऊक ऑर्डर: 20 किलो. इतर खरेदीदारांच्या ऑर्डरपासून बचावासाठी चेकआउटवर साठा पुन्हा तपासला जातो.",
  "Platform-coordinated delivery": "प्लॅटफॉर्म-समन्वित वितरण",
  "route estimate only": "केवळ मार्ग अंदाज",
  "More from the market board": "बाजार फलकावरील आणखी",
  "You may also need": "तुम्हाला हेही लागू शकते",
  "Similar quality, nearby harvests, and complementary produce.":
    "समान गुणवत्ता, जवळची काढणी आणि पूरक शेतमाल.",
  Placed: "नोंद",
  "Confirmed by farmer": "शेतकऱ्याने पुष्टी केली",
  "Harvest & pack": "काढणी व पॅकिंग",
  "Handed to transporter": "वाहतूकदाराकडे सुपूर्द",
  "In transit": "मार्गात",
  Delivered: "वितरित",
  Completed: "पूर्ण",
  "Awaiting update": "अपडेट प्रतीक्षेत",
  "Produce variety": "शेतमालाचा प्रकार",
  Unit: "एकक",
  Grade: "प्रत",
  "Available from": "उपलब्ध (पासून)",
  "Available until": "उपलब्ध (पर्यंत)",
  "Expected price / kg": "अपेक्षित किंमत / किलो",
  "Publish listing": "सूची प्रसिद्ध करा",
  "Quantity must be greater than zero.": "प्रमाण शून्यापेक्षा जास्त हवे.",
  "Available-until must be after available-from and cannot be in the past.":
    "उपलब्ध-पर्यंत तारीख उपलब्ध-पासूननंतर हवी व भूतकाळातील नको.",
  "Regional suggested price: ₹30/kg": "प्रादेशिक सुचवलेली किंमत: ₹30/किलो",
  "Choose language": "भाषा निवडा",
  Cart: "टोपली",
  "Your produce is being packed": "तुमचा शेतमाल पॅक होत आहे",
  "Collection expected 14:30–15:15": "संकलन अपेक्षित 14:30–15:15",
  "A transporter will be assigned after the farmer marks your order ready.":
    "शेतकऱ्याने तुमची ऑर्डर तयार म्हणून नोंदवल्यावर वाहतूकदार नेमला जाईल.",
  "DELIVERY ADDRESS": "वितरण पत्ता",
  "Route shown is an estimate, not live GPS.":
    "दाखवलेला मार्ग अंदाज आहे, लाइव्ह GPS नाही.",
  "Publish harvest availability": "पीक उपलब्धता प्रसिद्ध करा",
  "FARMER DESK / NEW LISTING": "शेतकरी डेस्क / नवीन सूची",
  "Farm location: Chevella, Telangana. Complete your profile before publishing if this is not correct.":
    "शेत स्थान: चेवेल्ला, तेलंगणा. हे बरोबर नसल्यास प्रसिद्धीपूर्वी प्रोफाइल पूर्ण करा.",
  "Agriculture field notes": "कृषी नोंदी",
  "Practical market, crop, and weather guidance for producers and buyers.":
    "उत्पादक व खरेदीदारांसाठी बाजार, पीक, हवामान व्यावहारिक मार्गदर्शन.",
  "{count} live listings · data updated 8 min ago":
    "{count} लाइव्ह सूची · 8 मिनिटांपूर्वी अपडेट",
  "No produce matches those filters.": "या फिल्टरशी कोणताही शेतमाल जुळत नाही.",
  "Broaden your location, price, or freshness filter to see live farm stock.":
    "लाइव्ह फार्म साठा पाहण्यासाठी स्थान, किंमत किंवा ताजेपणा फिल्टर विस्तारा.",
  Delete: "हटवा",
  "Delete this listing permanently? This cannot be undone.":
    "ही सूची कायमची हटवायची? ही क्रिया परत घेता येणार नाही.",
  "No listings yet. Publish your first harvest to appear here.":
    "अद्याप कोणत्याही सूची नाहीत. येथे दिसण्यासाठी तुमचे पहिले पीक प्रसिद्ध करा.",
  "Price must be greater than zero.": "किंमत शून्यापेक्षा जास्त हवी.",
  Yours: "तुमचे",
  Category: "प्रकार",
  "Clear cart": "टोपली रिकामी करा",
  "Continue shopping": "खरेदी सुरू ठेवा",
  "{count} items": "{count} वस्तू",
  "Delivery slot": "वितरण स्लॉट",
  "Decrease quantity": "प्रमाण कमी करा",
  "Increase quantity": "प्रमाण वाढवा",
  "Remove from cart": "टोपलीतून काढा",
  "Payment did not complete. Your order is still placed; retry safely.":
    "पेमेंट पूर्ण झाले नाही. तुमची ऑर्डर कायम आहे; सुरक्षितपणे पुन्हा प्रयत्न करा.",
  Analytics: "विश्लेषण",
  "View analytics": "विश्लेषण पहा",
  "Stock value overview": "साठा मूल्य आढावा",
  "Live market data across every visible listing.":
    "प्रत्येक दृश्य सूचीचा लाइव्ह बाजार डेटा.",
  "Total stock value": "एकूण साठा मूल्य",
  "Paddy stock value": "भात साठा मूल्य",
  "Vegetables stock value": "भाज्या साठा मूल्य",
  "Share of total stock": "एकूण साठ्यातील वाटा",
  "Paddy vs vegetables": "भात वि. भाज्या",
  "Other produce": "इतर शेतमाल",
  "Stock value by category": "प्रकारानुसार साठा मूल्य",
  "Top listings by stock value": "साठा मूल्यानुसार शीर्ष सूची",
  Value: "मूल्य",
  "AI assistant": "AI सहाय्यक",
  "Ask about prices, delivery, orders…": "किंमत, वितरण, ऑर्डरबद्दल विचारा…",
  "Online · replies instantly": "ऑनलाइन · त्वरित उत्तर",
  "Paddy prices": "भाताचे भाव",
  "How to sell": "कसे विकावे",
  "Report this listing": "या सूचीची तक्रार करा",
  Reported: "तक्रार केली",
  "Why are you reporting this listing?": "तुम्ही या सूचीची तक्रार का करत आहात?",
  "Wrong price": "चुकीची किंमत",
  "Poor quality": "निकृष्ट दर्जा",
  "Sold out": "साठा संपला",
  "Spam or fraud": "स्पॅम किंवा फसवणूक",
  "Tell us more (optional)": "अधिक सांगा (पर्यायी)",
  Cancel: "रद्द करा",
  "Submit report": "तक्रार सादर करा",
  "Thanks — our team will review this listing.":
    "धन्यवाद — आमची टीम या सूचीचे पुनरावलोकन करेल.",
};

export const dictionaries: Record<LanguageCode, Dict> = {
  en: {},
  hi,
  te,
  ta,
  kn,
  mr,
};

export function translate(
  lang: LanguageCode,
  key: string,
  vars?: Record<string, string | number>,
): string {
  let text = dictionaries[lang]?.[key] ?? key;
  if (vars) {
    for (const [k, v] of Object.entries(vars)) {
      text = text.replaceAll(`{${k}}`, String(v));
    }
  }
  return text;
}

export function isLanguageCode(value: string | null): value is LanguageCode {
  return (
    value === "en" ||
    value === "hi" ||
    value === "te" ||
    value === "ta" ||
    value === "kn" ||
    value === "mr"
  );
}
